    ...
    <div className="min-h-screen bg-[#050505] py-8 px-4">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="text-center mb-10">
          <div className="w-16 h-16 bg-gradient-to-br from-yellow-500 to-amber-600 flex items-center justify-center mx-auto mb-4">
            <Zap className="w-8 h-8 text-white" />
          </div>
          <h1 className="text-3xl md:text-4xl font-space font-bold text-white mb-2">KPLC Meter Transfer</h1>
          <p className="text-zinc-400">Process and costs to transfer electricity meter ownership</p>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
            <div className="bg-white/5 backdrop-blur-xl border border-white/10 p-6">
              ...
              {/* Transfer Type */}
              <div className="mb-6">
                <label className="block text-sm text-zinc-400 mb-3">What do you need?</label>
                  ...
                  {TRANSFER_TYPES.map((type) => (
                    ...
                    >
                      <div className="flex justify-between items-start">
                        ...
                        <span className={`font-semibold ${transferType === type.id ? 'text-yellow-400' : 'text-zinc-400'}`}>
              ...
              <div className="mb-4">
                <label className="flex items-center gap-3 cursor-pointer">
                  ...
                  />
                  <span className="text-zinc-300">Previous owner has outstanding bill</span>
                </label>
              </div>

              {hasOutstandingBill && (
                <div>
                  <label className="block text-sm text-zinc-400 mb-2">Outstanding Amount (KES)</label>
                  <input
                    type="number"
                    value={outstandingAmount}
                    onChange={(e) => setOutstandingAmount(Math.max(0, Number(e.target.value)))}
                    className="w-full bg-white/10 border border-white/20  px-4 py-3 text-white font-semibold focus:outline-none focus:border-yellow-500"
                  />
          ...
          {/* Results Section */}
          <div className="bg-gradient-to-br from-yellow-600/20 to-amber-900/20 backdrop-blur-xl  border border-yellow-500/20 p-6">
            <h2 className="text-lg font-semibold text-white mb-6">Cost Summary</h2>

            <div className="space-y-3">
              <div className="flex justify-between items-center py-3 border-b border-white/10">
                <span className="text-zinc-400">{selectedType.name} Fee</span>
                <span className="text-white font-semibold">{formatCurrency(selectedType.fee)}</span>
              </div>

              {hasOutstandingBill && outstandingAmount > 0 && (
                <div className="flex justify-between items-center py-3 border-b border-white/10">
                  <span className="text-zinc-400">Clear Outstanding Bill</span>
              ...
              <div key={step.step} className="bg-white/5 backdrop-blur-xl border border-white/10 p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-zinc-500 text-xs mt-2">⏱️ {step.duration}</p>
                  </div>
                </div>
              </div>
            ))}
