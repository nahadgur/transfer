    ...
    <div className="min-h-screen bg-[#050505] py-8 px-4">
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
          {[
            { label: 'Paybill', value: bank.paybill, color: 'text-[#ccff00]' },
            { label: 'USSD', value: bank.ussdCode, color: 'text-blue-400' },
            { label: 'M-Pesa→Bank Fee', value: bank.mpesaToBank.fee.includes('Free') ? 'Free' : bank.mpesaToBank.fee, color: 'text-[#ccff00]' },
            { label: 'Processing', value: bank.mpesaToBank.processingTime, color: 'text-amber-400' },
          ].map((fact) => (
            <div key={fact.label} className="bg-white/[0.02] border border-white/10 p-3 text-center">
              <div className={`font-bold text-sm ${fact.color}`}>{fact.value}</div>
              <div className="text-zinc-600 text-xs mt-0.5">{fact.label}</div>
            </div>
          ))}
        </div>

        {/* ── SECTION 1: M-Pesa → Bank ── */}
        <div className="border border-[#ccff00]/20  overflow-hidden mb-6">
          <div className="px-5 py-4 border-b border-[#ccff00]/20 flex items-center gap-2">
            <div className="w-7 h-7 bg-[#ccff00]/20 rounded-lg flex items-center justify-center">
              <span className="text-[#ccff00] text-xs font-bold">1</span>
            </div>
            <h2 className="text-white font-bold">M-Pesa → {bank.name}</h2>
            <span className="ml-auto text-xs text-[#ccff00] bg-[#ccff00]/10 px-2 py-0.5 rounded-full">
          ...
          <div className="p-5">
            ...
            <div className="grid sm:grid-cols-2 gap-3 mb-4 text-sm">
              <div className="bg-white/5  p-3">
                <div className="text-zinc-500 text-xs mb-1">Account Format</div>
                <div className="text-zinc-300">{bank.mpesaToBank.accountFormat}</div>
              </div>
              <div className="bg-white/5  p-3">
                <div className="text-zinc-500 text-xs mb-1">Amount Limits</div>
            ...
            {bank.mpesaToBank.notes.map((note, i) => (
              <div key={i} className="flex items-start gap-2 text-xs text-zinc-500 mb-1.5">
        ...
        <div className="border border-blue-500/20  overflow-hidden mb-6">
          ...
          <div className="p-5">
            ...
            <div className="grid sm:grid-cols-2 gap-3 mb-4 text-sm">
              <div className="bg-white/5  p-3">
                <div className="text-zinc-500 text-xs mb-1">Processing Time</div>
              ...
              <div className="bg-white/5  p-3">
                <div className="text-zinc-500 text-xs mb-1">Max Per Transaction</div>
                <div className="text-zinc-300">KES {bank.bankToMpesa.maxAmount.toLocaleString()}</div>
              </div>
            </div>
        ...
        {/* Customer care */}
        <div className="bg-white/[0.02] border border-white/10 p-4 flex items-center gap-4 mb-8">
          ...
          <div>
            ...
            <div className="text-zinc-400 text-sm">{bank.customerCare} · Available Mon–Sat</div>
          </div>
        </div>
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            {otherBanks.map((b) => (
              <Link
                ...
                className="flex items-center gap-3 bg-white/5 border border-white/10  px-4 py-3 hover:border-[#ccff00]/30 transition-colors group"
              >
                <BankIcon initial={b.initial} bgColor={b.bgColor} size="sm" />
                <span className="text-zinc-300 text-sm group-hover:text-white transition-colors flex-1">
                  M-Pesa ↔ {b.name}
                </span>
                <ArrowRight className="w-3.5 h-3.5 text-zinc-600 group-hover:text-[#ccff00] transition-colors" />
              </Link>
            ))}
          </div>
        </div>
