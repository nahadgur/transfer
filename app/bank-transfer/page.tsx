    ...
    <div className="min-h-screen bg-[#050505] py-8 px-4">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="text-center mb-10">
          ...
          <h1 className="text-3xl md:text-4xl font-space font-bold text-white mb-2">Bank Transfer Comparison</h1>
          <p className="text-zinc-400">Compare PesaLink, RTGS, and EFT fees across Kenya</p>
        </div>

        {/* Amount Input */}
        <div className="bg-white/5 backdrop-blur-xl border border-white/10 p-6 mb-8">
          <label className="block text-sm text-zinc-400 mb-3">Transfer Amount (KES)</label>
          <input
            type="number"
            value={amount}
            onChange={(e) => setAmount(Math.max(1, Number(e.target.value)))}
            className="w-full bg-white/10 border border-white/20  px-4 py-3 text-white text-2xl font-semibold focus:outline-none focus:border-blue-500 mb-4"
          />
          <div className="flex flex-wrap gap-2">
            {[10000, 50000, 100000, 500000, 1000000, 5000000].map((quickAmount) => (
              <button
                ...
                className="px-3 py-1.5 bg-white/5 rounded-lg text-zinc-400 text-sm hover:bg-white/10 transition-colors"
              >
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-12">
          {results.map((result, index) => (
            <div
              key={result.method}
              className={`relative bg-white/5 backdrop-blur-xl  border p-6 ${
                index === 0 ? 'border-[#ccff00]/50 ring-2 ring-[#ccff00]/20' : 'border-white/10'
              }`}
            >
              {index === 0 && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-1 bg-[#ccff00] rounded-full text-xs font-semibold text-white">
                  CHEAPEST
                </div>
              )}
              ...
              <div className="text-3xl font-space font-bold text-white mb-4">
                {result.fee === 0 ? (
              ...
              <div className="space-y-3 mb-6">
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <span className="text-zinc-300">{result.speed}</span>
                </div>
                <div className="flex items-center gap-2 text-sm">
                  <Zap className="w-4 h-4 text-amber-400" />
                  <span className="text-zinc-300">{result.limit}</span>
                </div>
              </div>

              <div className="border-t border-white/10 pt-4">
                <p className="text-xs text-zinc-500 uppercase tracking-wider mb-2">Pros</p>
                ...
                <p className="text-xs text-zinc-500 uppercase tracking-wider mb-2">Cons</p>
        ...
        {/* Recommendation */}
        <div className="backdrop-blur-xl  border border-blue-500/20 p-6">
          ...
          <div className="text-zinc-300">
            ...
            {amount >= 10000 && amount < 100000 && (
              <p><strong className="text-blue-400">Use PesaLink</strong> if you need instant transfer, or <strong className="text-[#ccff00]">EFT</strong> if you can wait 1-3 days and want to save on fees.</p>
            )}
            {amount >= 100000 && amount < 1000000 && (
              <p><strong className="text-[#ccff00]">EFT</strong> is cheapest for this amount. Use <strong className="text-blue-400">RTGS</strong> if you need same-day settlement during business hours.</p>
            )}
            {amount >= 1000000 && (
